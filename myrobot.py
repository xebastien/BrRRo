...
open_time  = "14:55" # does not consider time zone - must do calculation yourself
close_time = "21:55"
alavancagem = 3
...
# %% get data and run robot
def get_data(time_frame):
    pairs = mySymbols
    pair_data = dict()
    for pair in pairs:
        utc_from = datetime(2021, 7, 1, tzinfo=pytz.timezone('America/Sao_Paulo'))
...
if __name__ == '__main__':
    ...
    # PF, PFpertocome, PF_Weights_noLev = conv_dfWeights(PF_weights,br2_df)
    # # convert weights to shares
    # allocation_share, rem_minv = convert_weight2shares(br2_df,PF_Weights_noLev)
